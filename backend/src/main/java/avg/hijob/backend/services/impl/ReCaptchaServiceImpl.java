...
public class ReCaptchaServiceImpl implements ReCaptchaService {
    ...
    @Override
    public ReCaptchaResponse verify(String response) {
        // Tạm thời bỏ qua xác minh reCAPTCHA trong quá trình phát triển
        log.info("Skipping ReCaptcha verification (dev mode)");

        ReCaptchaResponse reCaptchaResponse = new ReCaptchaResponse();
        reCaptchaResponse.setSuccess(true);
        reCaptchaResponse.setScore(1.0f); // Giả lập điểm cao
